...
async fn test_top_up_idempotency() {
    ...
    let count = payego::schema::transactions::table
        .filter(payego::schema::transactions::reference.eq(reference))
        .count()
...
async fn test_internal_transfer_idempotency() {
    ...
    let recipient_email = "recipient@example.com";
    {
        ...
        diesel::insert_into(payego::schema::users::table)
            .values((
                payego::schema::users::email.eq(recipient_email),
                payego::schema::users::password_hash.eq("hashed"),
                payego::schema::users::username.eq("recipient"),
            ))
            .execute(&mut conn)
            .unwrap();
        ...
        diesel::update(payego::schema::wallets::table)
            ...
            .set(payego::schema::wallets::balance.eq(10000)) // $100
            .execute(&mut conn)
            .unwrap();
    }
    ...
    let count = payego::schema::transactions::table
        .filter(payego::schema::transactions::reference.eq(reference))
        .filter(payego::schema::transactions::user_id.eq(sender_id))
        .count()
        .get_result::<i64>(&mut conn)
        .unwrap();
    ...
    let balance: i64 = payego::schema::wallets::table
        .filter(payego::schema::wallets::user_id.eq(sender_id))
        .filter(payego::schema::wallets::currency.eq("USD"))
